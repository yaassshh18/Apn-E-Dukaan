                        ...
                        {cart.items.map(item => (
                            <div key={item.id} className="glass-card p-4 sm:p-6 flex flex-col sm:flex-row items-center gap-6 hover:shadow-hover transition-shadow bg-white text-left relative overflow-hidden group">
                                ...
                                <div className="w-full sm:w-32 h-32 bg-gray-100 rounded-xl overflow-hidden shrink-0 shadow-sm">
                                    {item.product?.image ? <img src={item.product.image} className="w-full h-full object-cover" /> : <div className="w-full h-full flex justify-center items-center text-gray-400 font-bold bg-gray-50 text-xs text-center border border-gray-100">No Image</div>}
                                </div>
                                <div className="flex-grow w-full text-center sm:text-left">
                                    <h3 className="font-display font-extrabold text-xl text-gray-900 mb-1">{item.product?.title}</h3>
